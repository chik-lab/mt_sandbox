...
def test_order_validator():
    ...
    run_log = BacktestLog(
        ...
        plan_name=plan.name,
        plan=plan,
        plan_strategy=plan.strategy_file,
        data=None,
        strategy_code=None,
        result=None,
        exception=None,
...
def test_ib_order_validator(launch_scheduler):
    ...
    plan.save()
    validator = InteractiveBrokersValidator(
        plan,
        broker,
        pytest_now=datetime(2023, 1, 3, 18, 0, tzinfo=ZoneInfo(plan.market_timezone)),
    )
    order = RawOrder(
        id="invalid",
        plan_id="invalid",
    ...
    with pytest.raises(AttributeError):
        MTDB.save(
            "IbTrade",
            {"execution_id": "an_id", "order_ref": order.id},
            on_conflict="update",
        )
        validator.order_not_in_finished_trades(order)
    with pytest.raises(AttributeError):
        MTDB.save(
            "IbOrder", {"orderId": "an_id", "order_ref": order.id}, on_conflict="update"
        )
    ...
    order = RawOrder(
        ...
        side="Buy",
        size=100,
        signal_time=datetime(2023, 1, 3, tzinfo=ZoneInfo(plan.market_timezone)),
        broker_order_id=None,
    )
    order.save()
    validator.validate(order)
