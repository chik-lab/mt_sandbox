...
class BrokerAccount:
    ...
    def get_account(plan_or_alias: TradePlan | str) -> BrokerAccount:
        ...
        alias = (
            plan_or_alias
            if isinstance(plan_or_alias, str)
            else plan_or_alias.broker_account
        )
...
class OrderValidator:
    ...
    def _assert_equal(self, value, expected_value, message):
        return self._assert(
            value == expected_value, f"{message}: {value=} != {expected_value=}"
        )

    def _assert_is_in(self, value, collection, message):
        return self._assert(
            value in collection, f"{message}: {value=} not in {collection=}"
        )
    ...
    def order_has_correct_run_id(self, order: RawOrder):
        self._assert_not_null(
            MTDB.get_one("BacktestLog", "id", order.run_id),
            "Order has incorrect run ID",
        )

    def order_has_correct_ticker(self, order: RawOrder):
        self._assert_is_in(
            order.ticker, self.plan.ticker_css.split(","), "Order has incorrect ticker"
        )
        self._assert_not_null(
            self.plan.broker_instrument_id(order.ticker),
            "Order has incorrect ticker mapping",
        )

    def order_has_correct_size(self, order: RawOrder):
        self._assert(
            (order.size > 0 and order.side == "Buy")
            or (order.size < 0 and order.side == "Sell"),
            "Order side and size does not agree",
        )
