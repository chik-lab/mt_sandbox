...
class InteractiveBrokers(Broker):
    ...
    def __call_ibgateway(
        ...
        if self._port:
            ...
            raise RuntimeError(
                f"IB gateway port is not set for request {method} {path}"
            )
    ...
    def is_ready(self) -> bool:
        ...
        try:
            status = self.__call_ibgateway_admin(
                "GET", f"/ibgateway/{self.account.alias}"
            )
    ...
    def connect(self):
        if not self.is_ready():
            try:
                status = self.__call_ibgateway_admin(
                    "PUT", f"/ibgateway/{self.account.alias}"
                )
                self._port = status["port"]
    ...
    def get_account_info(self) -> Any:
        ...
        for account in accounts:
            account["ledger"] = self.__call_ibgateway(
