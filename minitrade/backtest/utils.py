...
def generate_random_portfolios(
    universe: list[str], k: int, limit: int = None
) -> list[tuple]:
...
def backtest_strategy_parameters(
    data: pd.DataFrame,
    strategy: Strategy,
    ...
    **kwargs: dict[str, Any],
    ...
    bt_args = {
        ...
        if k in ["rebalance_tolerance", "rebalance_cash_reserve", "lot_size"]
    }
    opt_args = {
        k: v
        for k, v in kwargs.items()
...
def calculate_trade_stats(
    data: pd.DataFrame, cash: int, orders: list[dict], holding: dict = {}
):
        ...
        for open_order in open_orders[:]:
            if open_order["ticker"] == order["ticker"]:
                if open_order["size"] * order["size"] < 0:
                    ...
                    closed_size = copysign(
                        min(abs(open_order["size"]), abs(order["size"])),
                        open_order["size"],
                    )
                    ...
                    trade = {
                        ...
                        "pnl": (order["entry_price"] - open_order["entry_price"])
                        * closed_size,
                        "return_pct%": (
                            (
                                (order["entry_price"] / open_order["entry_price"] - 1)
                                * copysign(1, closed_size)
                                * 100
                            )
                            if open_order["entry_price"]
                            else None
                        ),
                    }
            ...
            open_orders.append(
                {
                    ...
                    "size": size,
                    "entry_price": close_prices[ticker].iloc[0],
                    "commission": 0,
                }
            )
    trades = []
    if orders:
        ...
        order_df = pd.DataFrame(
            columns=["ticker", "entry_time", "size", "entry_price", "commission"]
        )
    ...
    for open_order in open_orders:
        ...
        alt_trade = {
            ...
            "return_pct%": (
                (
                    (exit_price / open_order["entry_price"] - 1)
                    * copysign(1, open_order["size"])
                    * 100
                )
                if open_order["entry_price"]
                else None
    ...
    for i in range(0, len(data)):
        ...
        cost = (
            issued_orders["size"].dot(issued_orders["entry_price"])
    ...
    cash_available = (
        cash
        + order_df["size"].dot(order_df["entry_price"])
        - order_df["commission"].sum()
    )

    # calculate ticker performance
    position_size = (
        trade_df[trade_df["exit_time"].isna()]
        .groupby("ticker")["size"]
        .sum()
        .rename("Position Size")
    )
...
def shuffle_ohlcv(data: pd.DataFrame, in_sync: bool = False, random_state: int = None):
    ...
    def to_ohlcv(df):
        ...
        df[["Open", "High", "Low"]] = (df[["Open_rr", "High_rr", "Low_rr"]] + 1).mul(
            df["Close"], axis=0
        )
    ...
    if in_sync:
        rr.iloc[1:] = rr.iloc[1:].sample(
            frac=1, random_state=random_state, ignore_index=True
        )
    else:
        rr.iloc[1:] = rr.iloc[1:].ta.apply(
            lambda x: x.sample(frac=1, ignore_index=True, random_state=random_state)
        )
    rr.index = data.index
