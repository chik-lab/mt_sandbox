...
from minitrade.backtest.core._stats import compute_drawdown_duration_peaks
from minitrade.backtest.core._util import _Array, _as_str, _Indicator, try_
from minitrade.backtest.core.lib import (
    OHLCV_AGG,
    SignalStrategy,
    TrailingStrategy,
    barssince,
...
from minitrade.backtest.core.test import EURUSD, GOOG, SMA
...
class TestBacktest(TestCase):
    ...
    def test_assertions(self):
        class Assertive(Strategy):
            def init(self):
                self.sma = self.I(SMA, self.data.Close.df, 10)

                self.transpose_invalid = self.I(
                    lambda: np.column_stack((self.data.Open, self.data.Close))
                )
                ...
                ), "Strategy.I not called"
                assert any(
                    [resampled_ind is x for x in self._indicators]
                ), "Strategy.I not called"
            ...
            def next(self, _FEW_DAYS=pd.Timedelta("3 days")):  # noqa: N803
                ...
                if crossover(self.sma, self.data.Close):
                    ...
                    trade = self.trades()[0]
                    if (
                        self.data.index[-1] - self.data.index[trade.entry_bar]
                        > _FEW_DAYS
                    ):
    ...
    def test_compute_drawdown(self):
        ...
        np.testing.assert_array_equal(
            durations, pd.Series([3, 2], index=[3, 5]).reindex(dd.index)
        )
        np.testing.assert_array_equal(
            peaks, pd.Series([7, 4], index=[3, 5]).reindex(dd.index)
        )
    ...
    def test_compute_stats(self):
        ...
        def almost_equal(a, b):
            try:
                return np.isclose(a, b, rtol=1.0e-8)
            except TypeError:
                return a == b
    ...
    def test_position_close_portion(self):
        class SmaCross(Strategy):
            def init(self):
                self.sma1 = self.I(SMA, self.data.Close.df, 10)
                self.sma2 = self.I(SMA, self.data.Close.df, 20)
    ...
    def test_check_adjusted_price_when_placing_order(self):
        ...
        self.assertRaises(ValueError, Backtest(SHORT_DATA, S, commission=0.02).run)
...
class TestStrategy(TestCase):
    ...
    def test_stop_limit_order_price_is_stop_price(self):
        ...
        self.assertListEqual(
            stats._trades.filter(like="Price").stack().tolist(), [107, 112]
        )
...
class TestOptimize(TestCase):
    def test_optimize(self):
        ...
        self.assertRaises(
            ValueError, bt.optimize, return_optimization=True, **OPT_PARAMS
        )
        ...
        default_maximize = (
            inspect.signature(Backtest.optimize).parameters["maximize"].default
        )
        res2 = bt.optimize(**OPT_PARAMS, maximize=lambda s: s[default_maximize])
        self.assertDictEqual(
            res.filter(regex="^[^_]").fillna(-1).to_dict(),
    ...
    def test_method_skopt(self):
        ...
        res, heatmap, skopt_results = bt.optimize(
            fast=range(2, 20),
            slow=np.arange(2, 20, dtype=object),
        ...
        self.assertEqual(
            heatmap.index.tolist(), heatmap.dropna().index.unique().tolist()
        )
    ...
    def test_multiprocessing_windows_spawn(self):
        ...
        assert stats1.filter(regex="^[^_]").equals(stats2.filter(regex="^[^_]")), (
            stats1,
            stats2,
        )
...
class TestPlot(TestCase):
    ...
    def test_preview(self):
        class Strategy(SmaCross):
            def init(self):
                ...
                self.a = self.I(SMA, self.data.Open.df, 5, overlay=False, name="ok")
    ...
    def test_wellknown(self):
        class S(Strategy):
            ...
            def next(self):
                ...
                if date == pd.Timestamp("Thu 19 Oct 2006"):
                    ...
                    self.sell(
                        stop=self.data.Low[-1],
                        limit=324.90,  # High from 14 Nov
                        size=200,
                    )

        bt = Backtest(GOOG, S, margin=0.1)
        stats = bt.run()
        ...
        assert trades[["EntryTime", "ExitTime"]].equals(
            pd.DataFrame(
                {
                    "EntryTime": pd.to_datetime(["2006-11-01", "2008-11-14"]),
                    "ExitTime": pd.to_datetime(["2007-10-31", "2009-09-21"]),
                }
            )
        )
        assert trades["PnL"].round().equals(pd.Series([23469.0, -34420.0]))
    ...
    def test_resample(self):
        ...
        with _tempfile() as f, patch.object(
            minitrade.backtest.core._plotting, "_MAX_CANDLES", 10
    ...
    def test_indicator_color(self):
        class S(Strategy):
            def init(self):
                a = self.I(SMA, self.data.Close.df, 5, overlay=True, color="red")
                b = self.I(SMA, self.data.Close.df, 10, overlay=False, color="blue")
    ...
    def test_indicator_circle(self):
        class S(Strategy):
            def init(self):
                ...
                self.I(SMA, self.data.Close.df, 10, overlay=False, scatter=True)
...
class TestLib(TestCase):
    ...
    def test_resample_apply(self):
        ...
        np.testing.assert_almost_equal(
            res.iloc[-48:].unique().tolist(), [1.242643, 1.242381, 1.242275], decimal=6
        )
    ...
    def test_compute_stats(self):
        ...
        long_stats = compute_stats(
            stats=stats, trades=only_long_trades, data=GOOG, risk_free_rate=0.02
        )
        self.assertNotEqual(
            list(stats._equity_curve.Equity), list(long_stats._equity_curve.Equity)
        )
...
class TestUtil(TestCase):
    def test_as_str(self):
        ...
        df = pd.DataFrame()
        df.attrs.update({"name": "x"})
        self.assertEqual(_as_str(df), "x")
    ...
    def test_pandas_accessors(self):
        class S(Strategy):
            def init(self):
                ...
                # self.data.df['new_key'] = 2 * close       # not supported
