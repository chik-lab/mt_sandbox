...
class CboeFuturesQuoteSource(QuoteSource):
    ...
    def _download_futures_history(self, futures_root, num_workers=8):
        ...
        def fetch_data(file):
            ...
            try:
                ...
                if cache["freeze"] or cache["update_time"] > now.replace(
                    hour=0, minute=0, second=0, microsecond=0
                ):
                    return itemgetter(*key_fields)(file), pd.read_csv(
                        StringIO(cache["data"])
                    )
            except Exception:
                pass
            data = requests.get(data_url.format(file["path"])).text
            freeze = datetime.strptime(
                file["expire_date"], "%Y-%m-%d"
            ).date() < now.date() - timedelta(days=3)
            MTDB.save(
                "CboeFuturesCache",
                file | {"data": data, "freeze": freeze, "update_time": now},
                on_conflict="update",
            )
        ...
        data_files = [
            x
            for year in requests.get(index_url.format(futures_root)).json().values()
            for x in year
        ]
        if not data_files:
        ...
        df = (
            ...
            .sort_values(["Trade Date", "expire_date"])
        )
        df = (
            df[df["duration_type"] == "M"]
            .copy()
            .rename(columns={"Trade Date": "date", "Total Volume": "Volume"})
        )
        return df
    ...
    def _continuous_front_month_futures(self, df, cf_method, roll_day=-5):
        ...
        if cf_method:
            ...
            roll_fill = (
                pd.concat(
                    [expire_index.shift(i) for i in range(roll_day + 1, 1)], axis=1
                )
                .any(axis=1)
                .reindex(df.index)
            )
            ...
            if cf_method == "proportional":
                adjust_ratio = (
                    df[roll_index]["M1", "Close"] / df[roll_index]["M0", "Close"]
                )
                adjust_ratio_fill = (
                    adjust_ratio.iloc[::-1]
                    .cumprod()
    ...
    def _futures_term_structure(self, df):
        ...
        df = (
            df.pivot_table(
                index="date", columns="Month", values=ohlcve, aggfunc="first"
            )[ohlcve]
            .swaplevel(axis=1)
            .sort_index(axis=1)
        )
        ...
        df = df.reindex(
            zip(
                ...
                ohlcve * len(df.columns.get_level_values(0)),
            ),
            axis=1,
        )
        return df
    ...
    def _format_ticker(self, ticker):
        ...
        try:
            root, month, year, root2, root3, expire_date = re.match(
                r"^([A-Z]+\d*)([FGHJKMNQUVXZ])(\d+)$|^([A-Z]+\d*)$|^([A-Z]+\d*)\@(\d{4}-\d{2}-\d{2})$",
                ticker,
            ).groups()
            ...
            expire_date = expire_date or (
                f"{year}-{month:02d}" if year and month else None
            )
