...
class EastMoneyQuoteSource(QuoteSource):
    ...
    def _daily_bar(self, ticker, start, end):
        try:
            df: pd.DataFrame = ak.stock_zh_a_hist(
                ...
                end_date=(
                    end.replace("-", "") if end else "20500101"
                ),  # magic number used in ak lib
                adjust="qfq",
            )
        except KeyError:
    ...
    def _spot(self, tickers):
        try:
            ...
            df = [
                f()
                .rename(columns={"代码": "ticker", "最新价": "price"})
                .set_index("ticker")
                for f in src
            ]
            s = pd.concat(df)["price"]
            data = {
                ticker: s[ticker] if ticker in s.index else None for ticker in tickers
            }
