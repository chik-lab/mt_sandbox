...
class YahooQuoteSource(QuoteSource):
    ...
    def _ticker_timezone(self, ticker):
        return yf.Ticker(self._format_ticker(ticker), proxy=self.proxy).fast_info[
            "timezone"
        ]
    ...
    def _ticker_calendar(self, ticker):
        ...
        exch = yf.Ticker(self._format_ticker(ticker), proxy=self.proxy).fast_info[
            "exchange"
        ]
        if exch in mapping:
            return mapping[exch]
    ...
    def _daily_bar(self, ticker, start, end):
        ...
        if (
            not df.loc[today:].empty or not end or end >= today.strftime("%Y-%m-%d")
        ) and self.is_trading_now(ticker):
            tz = df.index.tzinfo
            ...
            df = pd.concat(
                [df, pd.DataFrame(row, index=[today.astimezone(timezone.utc)])]
            )
    ...
    def _spot(self, tickers):
        try:
            data = {
                ticker: (
                    yf.Ticker(self._format_ticker(ticker), proxy=self.proxy).fast_info[
                        "lastPrice"
                    ]
                    if self.is_trading_now(ticker)
                    else None
                )
