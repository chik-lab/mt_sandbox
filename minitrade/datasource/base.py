...
class QuoteSource(ABC):
    ...
    def today(self, ticker: str) -> datetime:
        ...
        return datetime.now(ZoneInfo(self.ticker_timezone(ticker))).replace(
        ...
        try:
            ...
            if align:
                ...
                if normalize:
                    ...
                    for s in tickers:
                        df.loc[:, (s, ohlc)] = (
                            df.loc[:, (s, ohlc)] / df[s].loc[start_index, "Close"]
                        )
            elif normalize:
    ...
    def monthly_bar(
        ...
        try:
            start = (
                ...
                .rollback(pd.to_datetime(start))
                .strftime("%Y-%m-%d")
            )
            end = (
                (pd.to_datetime(end) + pd.offsets.MonthEnd(1)).strftime("%Y-%m-%d")
                if end
                else None
            )
            daily = self.daily_bar(
                tickers, start, end, align, normalize, num_workers=num_workers
            )
            monthly = daily.ta.apply(
                lambda x: x.resample("ME").agg(
                    {
                        "Open": "first",
                        "High": "max",
                        "Low": "min",
