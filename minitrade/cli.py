...
def check_program_version(name, path=None):
    try:
        if not path:
            proc = subprocess.run(
                ["which", name], capture_output=True, cwd=os.getcwd(), text=True
            )
            if proc.returncode:
                raise RuntimeError(f"Program {name} not found")
            path = proc.stdout.strip("\n")
        proc = subprocess.run(
            [path, "--version"], capture_output=True, cwd=os.getcwd(), text=True
        )
        if proc.returncode:
            raise RuntimeError(f"Program {name} not found")
...
def ib_diagnose():
    ...
    for item in items:
        ...
        click.secho(
            f'  {item:35s} ... {"found" if path.exists() else "missing"}',
    ...
    try:
        if (
            requests.get(
                f"http://localhost:{config.scheduler.port}/strategy", timeout=3
            ).status_code
            != 200
        ):
    ...
    try:
        if (
            requests.get(
                f"http://localhost:{config.brokers.ib.gateway_admin_port}/ibgateway",
                timeout=3,
            ).status_code
            == 200
        ):
            click.secho(
                "  IB gateway is already running. Quit it and try again.", fg="red"
            )
            return
    except Exception:
        pass
    try:
        ...
        res = requests.put(
            f"http://localhost:{config.brokers.ib.gateway_admin_port}/ibgateway/{alias}"
        )
...
@mtcli.command()
@click.option(
    "-y", "--yes", is_flag=True, help="Don't ask for confirmation of inplace upgrade"
)
def init(yes):
...
def build_db_schema(minitrade_root):
    ...
    sqlfiles = [
        f
        for f in os.listdir(os.path.join(os.path.dirname(__file__), "trader/schema"))
        if f.endswith(".sql")
    ]
    sqlversions = [
        tuple(int(_) for _ in f.removesuffix(".sql").split("."))
        for f in sqlfiles
        if f != "consolidated.sql"
    ]
    ...
    for sqlversion in sorted(sqlversions):
        ...
        sql = pkgutil.get_data(__name__, f"trader/schema/{sqlversion}.sql").decode(
            "utf-8"
        )
        sql = (
            "BEGIN TRANSACTION;"
            + sql
            + f'UPDATE schema_version SET version="{sqlversion}";'
            + "COMMIT;"
        )
        with closing(sqlite3.connect(db_loc)) as conn:
...
def install_ibgateway(minitrade_root):
    try:
        ...
        with open(f"{ib_loc}/clientportal.gw.zip", "wb") as f:
            ...
            t = tqdm(
                response.iter_content(chunk_size=1000),
                total=total_kb,
                unit="KB",
                desc="Downloading IB gateway",
                leave=False,
            )
            for data in t:
