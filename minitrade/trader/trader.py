...
class TradePlan:
    ...
    def get_plan(plan_id_or_name: str) -> TradePlan:
        ...
        """
        plan: TradePlan = MTDB.get_one(
            "TradePlan", "id", plan_id_or_name, cls=TradePlan
        ) or MTDB.get_one("TradePlan", "name", plan_id_or_name, cls=TradePlan)
        if plan:
            plan.load_storage()
        return plan

    def __call_scheduler(
        ...
        if resp.status_code == 200:
            ...
            raise RuntimeError(
                f"Scheduler {method} {url} {params} returns {resp.status_code} {resp.text}"
            )
    ...
    def enable(self, enable: bool):
        ...
        MTDB.update(
            ...
            "id",
            self.id,
            values={"enabled": self.enabled, "update_time": self.update_time},
        )
    ...
    def get_orders(self, run_id: str = None) -> list[RawOrder]:
        ...
        if run_id:
            return MTDB.get_all(
                "RawOrder", where={"plan_id": self.id, "run_id": run_id}, cls=RawOrder
            )
        else:
...
class StrategyManager:
    ...
    def list() -> list[str]:
        ...
        return [
            f
            for f in os.listdir(StrategyManager.__strategy_root)
            if os.path.isfile(StrategyManager._path(f))
        ]
...
class BacktestRunner:
    ...
    def _check_data(self, data: pd.DataFrame):
        ...
        if len(data) > 1:
            same = np.isclose(
                data.iloc[-1].loc[:, ohlc].astype(float),
                data.iloc[-2].loc[:, ohlc].astype(float),
                rtol=1e-5,
            )
            if same.any():
    ...
    def run_backtest(
        ...
        Args:
            ...
            clear_previous: If True, clear all previous records in output/simulations and output/logs
                before starting the new simulation. Defaults to True.
        ...
        exception = None
        try:
            # Clear previous simulation records if requested
            if clear_previous:
                try:
            ...
            if self.plan.data_source:
                ...
                self.data = source.daily_bar(
                    tickers=self.plan.ticker_css, start=self.plan.backtest_start_date
                )
            else:
                self.data = self.strategy.prepare_data(
                    self.plan.ticker_css.split(","), self.plan.backtest_start_date
                )
            self._check_data(self.data)
            if self.plan.strict:
                bt = _record_backtest_params(
    ...
    def execute(self, dryrun: bool = False) -> BacktestLog:
        ...
        try:
            ...
            proc = subprocess.run(
                ...
                + (["--pytest"] if "pytest" in sys.modules else []),
                capture_output=True,
                cwd=os.getcwd(),
                timeout=3600,
                env=env,
            ...
            raise RuntimeError(
                f"Backtest for {self.plan} run {self.run_id} failed"
            ) from e
        finally:
    ...
    def _log_backtest_run(
        ...
        if log is None:
            ...
            MTDB.update(
                ...
                values={
                    "exception": "\n\n".join(
                        [x for x in (log.exception, exception) if x]
                    )
                    or None,
                    "stdout": stdout,
                    "stderr": stderr,
    ...
    def _send_backtest_notification(self):
        ...
        if log:
            ...
            plan_time = (
                log.log_time.replace(tzinfo=ZoneInfo("UTC"))
                .astimezone(ZoneInfo(self.plan.market_timezone))
                .strftime("%Y-%m-%d %H:%M:%S")
            )
            plan_subject = f"## {plan_name} ##  {plan_status} @ {plan_time}"
            plan_positions = plan_data = plan_result = plan_exception = plan_stderr = (
                plan_stdout
            ) = None
            ...
            if log.result is not None:
                if "_positions" in log.result.index:
                    ...
                    plan_positions = tabulate(
                        positions.items(), headers=["Ticker", "Share"]
                    )
                result = log.result
                result = result[~result.index.str.startswith("_")].T.apply(
            ...
            message.append(
                f'<pre>{html.escape(tabulate(summary, headers="keys"))}</pre>'
            )

            if orders:
                plan_orders = tabulate(
                    [[o.side, o.ticker, abs(o.size)] for o in orders]
                )
            ...
            silent = (
                not log.error
                and not orders
                and len(self.plan.trade_time_of_day.split(",")) > 1
            )
...
class Trader:
    ...
    def submit_orders(self, plan: TradePlan):
        ...
        orders = MTDB.get_all(
            "RawOrder",
            where={"plan_id": plan.id, "broker_order_id": None, "cancelled": False},
            orderby="signal_time",
            cls=RawOrder,
        )
    ...
    def _log_trader_run(
        self, run_id: str, summary: list[str], start_time: datetime
    ) -> None:
        """Log the input and output of a trader run to database."""
        text = "\n\n".join(summary)
        log = TraderLog(
            id=run_id, summary=text, start_time=start_time, log_time=datetime.utcnow()
        )
        log.save()
        send_telegram_message(html=text)
        mailjet_send_email(
            f'Trader @ {start_time.replace(tzinfo=ZoneInfo("UTC")).astimezone().strftime("%Y-%m-%d %H:%M:%S")}',
            text,
        )
...
class TaskPlan:
    ...
    def get_plan(plan_id_or_name: str) -> TaskPlan:
        ...
        """
        plan: TaskPlan = MTDB.get_one(
            "TaskPlan", "id", plan_id_or_name, cls=TaskPlan
    ...
    def enable(self, enable: bool):
        ...
        if enable:
            if not self.schedule or not self.timezone:
                raise RuntimeError(
                    "Task plan must have schedule and timezone to be enabled"
                )
        self.enabled = enable
        self.update_time = datetime.utcnow()
        MTDB.update(
...
class TaskManager:
    ...
    def list() -> list[str]:
        ...
        return [
            f
            for f in os.listdir(TaskManager.__task_root)
            if os.path.isfile(TaskManager.locate(f))
        ]
