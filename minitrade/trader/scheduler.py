...
def run_trader_after_backtest(
    plan: TradePlan, force: bool = False
) -> BacktestLog | None:
    calendar = mcal.get_calendar(plan.market_calendar)
    today = datetime.now(ZoneInfo(plan.market_timezone)).replace(
        hour=0, minute=0, second=0, microsecond=0
    )
    trading_days = calendar.valid_days(
        start_date=(today - timedelta(days=30)).strftime("%Y-%m-%d"),
        end_date=today.strftime("%Y-%m-%d"),
        tz=plan.market_timezone,
    )
    ...
    if plan.enabled:
        try:
            trade_time = [
                ...
                for t in plan.trade_time_of_day.split(",")
            ]
            trigger = OrTrigger(
                [
                    CronTrigger(
            ...
            raise HTTPException(
                500, f'Failed to schedule trade plan "{plan.name}": {e}'
            )
    else:
        try:
...
def get_strategy():
    ...
    return [
        jobinfo(job) for job in trade_scheduler.get_jobs() if job.id != "trader_runner"
    ]
...
async def post_messages(data: Message):
    ...
    bot and await bot.send_message(
        data.html or data.text,
        parse_mode="HTML" if data.html else None,
        silent=data.silent,
    )
    return Response(status_code=204)
