...
def send_telegram_message(*text, html: str = "", silent: bool = False):
    ...
    if "pytest" not in sys.modules:
        ...
        resp = requests.request(
            method="POST", url=url, json={"text": text, "html": html, "silent": silent}
        )
...
class TelegramBot:
    ...
    def __init__(self, token: str = None, chat_id: str = None, proxy: str = None):
        if token or config.providers.telegram.token:
            ...
            self.app = (
                ApplicationBuilder()
                .token(token or config.providers.telegram.token)
                .proxy_url(self.proxy)
                .get_updates_proxy_url(self.proxy)
                .build()
            )
    ...
    async def __call_ibgateway_admin(
        self, method: str, path: str, params: dict | None = None
    ):
        ...
        if resp.status_code == 200:
            ...
            raise RuntimeError(
                f"Request {path} returned {resp.status_code} {resp.text}"
            )
        ...
        if cmd == "enable":
            ...
            if plan:
                ...
                await context.bot.send_message(
                    chat_id=update.effective_chat.id, text=f'"{plan_name}" enabled'
                )
            else:
                await context.bot.send_message(
                    chat_id=update.effective_chat.id, text=f'"{plan_name}" not found'
                )
        elif cmd == "disable":
            plan = await asyncio.to_thread(TradePlan.get_plan, plan_name)
            if plan:
                ...
                await context.bot.send_message(
                    chat_id=update.effective_chat.id, text=f'"{plan_name}" disabled'
                )
            else:
                await context.bot.send_message(
                    chat_id=update.effective_chat.id, text=f'"{plan_name}" not found'
            ...
            if jobs:
                await context.bot.send_message(
                    ...
                    text=f"{len(jobs)} strategy scheduled",
                )
                for j in jobs:
    ...
    async def task(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if cmd == "enable":
            ...
            if jobs:
                await context.bot.send_message(
                    chat_id=update.effective_chat.id, text=f"{len(jobs)} task scheduled"
                )
                for j in jobs:
                    status = f'"{j["job_name"]}" next run @ {j["next_run_time"]}'
                    await context.bot.send_message(
            ...
            async def login():
                for account in BrokerAccount.list():
                    ...
                    try:
                        ...
                        await context.bot.send_message(
                            chat_id=update.effective_chat.id,
                            text=f"Login {account.alias} ... OK",
                        )
                    except Exception:
                        await context.bot.send_message(
                            chat_id=update.effective_chat.id,
                            text=f"Login {account.alias} ... ERROR",
                        )
                await self.__call_scheduler("PUT", "/trader")

            self.ib_login_task = asyncio.create_task(login(), name="login")

            def clear_ib_login_task(x):
                self.ib_login_task = None

            self.ib_login_task.add_done_callback(clear_ib_login_task)
        else:
            status = await self.__call_ibgateway_admin("GET", "/ibgateway")
            await context.bot.send_message(
                chat_id=update.effective_chat.id, text=f"{len(status)} gateway running"
            )
            for s in status:
                await context.bot.send_message(
                    chat_id=update.effective_chat.id, text=str(s)
                )
    ...
    async def unknown(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
            text="Sorry, I don't understand that command. Try /help.",
        )
    ...
    async def send_message(
        self, text: str, parse_mode: str = None, silent: bool = False
    ):
        if self.chat_id:
