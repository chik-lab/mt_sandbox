...
def display_process_status():
    ...
    pid_map = {
        pid: name
        for pid, ppid, _, _, name, *_ in [line.split() for line in output.splitlines()]
        if ppid == "1"
    }
    ...
    for k, v in proc_map.items():
        if len(v) == 0:
            ...
            st.text(
                f'Process "{k}" is running, pid {v}'
                + (f", child pid {child_pid}." if len(child_pid) > 0 else ".")
            )
        else:
            st.error(
    ...
    with closing(MTDB.conn()) as conn:
        stats = {
            table: (
                ...
                conn.execute(
                    f'SELECT SUM("pgsize")/1024.0/1024.0 FROM "dbstat" WHERE name="{table}"'
                ).fetchone()[0],
            )
            for table in tables
        }
