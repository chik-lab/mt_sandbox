...
def test_and_save_yahoo_proxy(proxy):
    ...
    df = QuoteSource.get_source("Yahoo", proxy=proxy).daily_bar(
        "SPY", start="2023-01-01"
    )
    if len(df) > 0:
        st.write(df)
...
def test_and_save_eodhd_api_key(api_key):
    st.caption("Getting SPY")
    df = QuoteSource.get_source("EODHistoricalData", api_key=api_key).daily_bar(
        "SPY", start="2023-01-01"
    )
    if len(df) > 0:
        st.write(df)
        ...
        st.error(
            "Getting data from EODHistoricalData not working, please check API key"
        )


def test_and_save_twelvedata_api_key(api_key):
    st.caption("Getting SPY")
    df = QuoteSource.get_source("TwelveData", api_key=api_key).daily_bar(
        "SPY", start="2023-01-01"
    )
    if len(df) > 0:
        st.write(df)
...
def test_and_save_alpaca_api_key(api_key, api_secret):
    ...
    df = QuoteSource.get_source(
        "Alpaca", api_key=api_key, api_secret=api_secret
    ).daily_bar("SPY", start="2023-01-01")
    if len(df) > 0:
        st.write(df)
...
def test_and_save_tiingo_api_key(api_key):
    st.caption("Getting SPY")
    df = QuoteSource.get_source("Tiingo", api_key=api_key).daily_bar(
        "SPY", start="2023-01-01"
    )
    if len(df) > 0:
        st.write(df)
...
def test_and_save_ib_account(alias):
    st.caption("Getting SPY")
    df = QuoteSource.get_source("InteractiveBrokers", alias=alias).daily_bar(
        "SPY", start="2023-01-01"
    )
    if len(df) > 0:
        st.write(df)
        ...
        st.error(
            "Getting data from InteractiveBrokers not working, please check account"
        )
...
def confirm_delete_union_data_source(source) -> None:
    ...
    st.text_input(
        f'Type "{source}" and press Enter to delete',
...
def compare_data_from_sources(data, s1, s2, tab):
    ...
    df = pd.merge(
        df1, df2, how="outer", left_index=True, right_index=True, suffixes=("_1", "_2")
    )
    cols = tab.columns(5)
...
def plot_ohlcv(data):
    for ticker in data.columns.get_level_values(0).unique():
        ...
        c1.dataframe(df)
        c1.download_button(
            f"Download {ticker}", df.to_csv().encode("utf-8"), f"{ticker}.csv"
        )
        # plot stats
        c2.write("### Stats")
        stats = df.describe()
...
def lookup_ticker():
    if not config.sources.ib.account:
        st.sidebar.warning(
            "Please make sure you have configured the IB account for data access."
        )
        return
