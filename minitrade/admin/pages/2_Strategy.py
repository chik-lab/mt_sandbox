...
def show_strategy_uploader():
    ...
    uploaded_file = st.sidebar.file_uploader(
        "Upload strategy file", type=["py"], accept_multiple_files=False
    )
    if uploaded_file is not None:
        StrategyManager.save(
            uploaded_file.name, uploaded_file.getvalue().decode("utf-8")
        )
...
def confirm_delete_trade_strategy(strategy_file: str) -> None:
    ...
    st.text_input(
        f'Type "{strategy_file}" and press Enter to delete',
