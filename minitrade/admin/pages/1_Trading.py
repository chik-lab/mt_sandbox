...
def get_market_calendar_and_timezone(data_source: str, ticker_css: str):
    ...
    calendars = list(
        dict.fromkeys([ds.ticker_calendar(t) for t in ticker_css.split(",")])
    )
            ...
            try:
                ...
                if candidates:
                    ...
                    for k, v in candidates.items():
                        vendor_tickers[k] = st.radio(
                            k, v, format_func=lambda _: _["label"]
                        )
                    return vendor_tickers
            except ConnectionError:
                st.error(
                    "Need to login to broker account to resolve tickers. Please pay attention to 2FA notification if enabled."
                )
                st.button("Retry")
    return {}
        ...
        for t in trade_time_of_day.split(","):
            ...
            if len(t) > 8:
                ...
                ts.append(
                    ...
                    if len(t.split(":")) == 3
                    else datetime.strptime(t, "%H:%M").time()
                )
...
def _get_market_config(data_source, ticker_css):
    ...
    try:
        if data_source:
            market_calendar, market_timezone = get_market_calendar_and_timezone(
                data_source, ticker_css
            )
    ...
    market_calendar = c1.selectbox(
        "Select market", options=markets, index=markets.index(market_calendar or "NYSE")
    )
    market_timezone = c2.selectbox(
...
def _get_trade_timing_config():
    ...
    entry_type = c1.selectbox(
        ...
        format_func=lambda x: {
            "TOO": "Trade on open (TOO)",
            "TOC": "Trade on close (TOC)",
            "TRG": "Trade regular hours (TRG)",
        }[x],
    )
    ...
    try:
        ...
        if trade_time_of_day:
            st.info(
                f'{len(trade_time_of_day.split(","))} runs @ {trade_time_of_day.replace(",", ", ")}'
            )
    except Exception as e:
        st.error(e)
...
def show_create_trade_plan_form() -> TradePlan | None:
    ...
    initial_holding = (
        st.text_input(
            ...
            placeholder="e.g. AAPL:100,GOOG:100",
        )
        or None
    )
...
def display_run(plan: TradePlan, log: BacktestLog):
    ...
    log_time = (
        log.log_time.replace(tzinfo=zoneinfo.ZoneInfo("UTC"))
        .astimezone(zoneinfo.ZoneInfo(plan.market_timezone))
        .strftime("%Y-%m-%d %H:%M:%S")
    )
    label = f"{log_status} {log_time}" + (
        f" **{len(orders)} orders**" if orders else ""
    )
...
def show_trade_plan_selector() -> TradePlan | None:
    ...
    plan_idx = st.sidebar.radio(
        "Trade plan", list(range(len(plan_lst))), format_func=lambda i: plan_lst[i].name
    )
...
def confirm_delete_trade_plan(plan: TradePlan) -> None:
    ...
    st.text_input(
        f'Type "{plan.name}" and press Enter to delete',
        on_change=confirm_delete,
        key="delete_confirm_textinput",
    )
...
def show_trade_plan_header_and_controls(plan: TradePlan) -> None:
    ...
    c4.button(
        "Disable" if plan.enabled else "Enable",
        key="tradeplan_onoff",
...
def show_trade_plan_status(plan: TradePlan) -> None:
    ...
    if job is None:
        if plan.enabled:
            st.warning(
                "State inconsistency detected: plan is enabled but not scheduled"
            )
        else:
...
def show_trade_plan_execution_history(plan: TradePlan) -> None:
    tab1, tab2, tab3, tab4 = st.tabs(
        ["Run history", "Orders", "Performance", "Settings"]
    )
    logs = plan.list_logs()
    with tab1:
...
def show_orders(plan, account, broker, orders):
    for order in orders:
        ...
        with st.expander(
            f"{order_status} {order.signal_time} **{order.ticker} {order.side} {abs(order.size)}**"
        ):
            ...
            if trade:
                ...
                if account.broker == "Manual":
                    ...
                    price = c1.number_input(
                        "Price", key=f"{order.id}-price", value=trade[0]["price"] or 0.0
                    )
                    commission = c2.number_input(
                        "Commission",
                        key=f"{order.id}-comm",
                        value=trade[0]["commission"] or 0.0,
                    )
                    dt = c3.date_input(
                        "Trade time (market local timezone)",
                        key=f"{order.id}-date",
                        value=trade[0]["trade_time"],
                    )
                    tm = c4.time_input(
                        "Trade time",
                        key=f"{order.id}-time",
                        value=trade[0]["trade_time"] or time(9, 30),
                    ...
                    if t3.button("Save", key=f"{order.id}-save"):
                        broker.update_trade(
                            ...
                            commission,
                            datetime.combine(
                                dt, tm, zoneinfo.ZoneInfo(plan.market_timezone)
                            ),
                        )
...
def show_performance(plan, logs, broker, orders):
    try:
        ...
        trade_start_date = datetime.strptime(plan.trade_start_date, "%Y-%m-%d").replace(
            tzinfo=zoneinfo.ZoneInfo(plan.market_timezone)
        )
        ...
        if len(equity) > 0:
            ...
            c1.metric(
                ...
                value=f"{equity.iloc[-1]:,.0f}",
                delta=(
                    f"{int(equity.iloc[-1]-equity.iloc[-2]):,}"
                    if len(equity) >= 2
                    else 0
                ),
            )
            c2.metric(
                label="PnL",
                value=f"{(equity.iloc[-1]-equity.iloc[0]):,.0f}",
            ...
            broker_orders["entry_time"] = broker_orders["entry_time"].dt.tz_convert(
                plan.market_timezone
            )
            broker_orders["value"] = (
                broker_orders["entry_price"] * broker_orders["size"]
            ).astype(int)
