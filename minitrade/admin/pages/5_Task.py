...
def show_task_uploader():
    ...
    uploaded_file = st.sidebar.file_uploader(
        "Upload task file", type=["py"], accept_multiple_files=False
    )
...
def show_schedule_editor(tab, plan: TaskPlan):
    ...
    timezone = tab.selectbox(
        "Schedule timezone",
        zones,
        index=zones.index(plan.timezone) if plan.timezone in zones else 0,
    )
...
def show_task_plan_status(tab, plan: TaskPlan) -> None:
    with tab:
        ...
        if job is None:
            if plan.enabled:
                st.warning(
                    "State inconsistency detected: task is enabled but not scheduled"
                )
            else:
    ...
    with tab:
        for log in plan.list_logs():
            ...
            log_time = (
                ...
                .astimezone(ZoneInfo(plan.timezone))
