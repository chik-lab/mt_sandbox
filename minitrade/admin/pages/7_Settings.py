...
def test_and_save_mailjet(api_key, api_secret, sender, mailto):
    if mailjet_send_email(
        "Mailjet is correctly configured",
        "Sent from Minitrade",
        api_key,
        api_secret,
        sender,
...
if provider == "Mailjet":
    ...
    api_key = (
        st.text_input("API key", value=config.providers.mailjet.api_key or "") or None
    )
    api_secret = (
        st.text_input("API secret", value=config.providers.mailjet.api_secret or "")
        or None
    )
    sender = (
        st.text_input("Sender email", value=config.providers.mailjet.sender or "")
        or None
    )
    mailto = (
        st.text_input("Recipient email", value=config.providers.mailjet.mailto or "")
        or None
    )
    if st.button("Save"):
...
if provider == "Telegram":
    ...
    chat_id = (
        st.text_input("Chat ID", value=config.providers.telegram.chat_id or "") or None
    )
    proxy = (
        st.text_input(
            "Proxy",
            placeholder="http://user:pass@host:port or https://user:pass@host:port or socks5://user:pass@host:port",
            value=config.providers.telegram.proxy or "",
        )
        or None
    )
    if st.button("Save"):
