...
def show_broker_account_selector():
    ...
    acc_idx = st.sidebar.radio(
        "Broker account",
        list(range(len(acc_lst))),
...
def confirm_delete_broker_account(account: BrokerAccount) -> None:
    ...
    st.text_input(
        f'Type "{account.alias}" and press Enter to delete',
...
def show_ib_account_overview(tab, info, portfolio):
    with tab:
        account = st.selectbox(
            "Account",
            info,
            format_func=lambda x: f'{x["displayName"]} ({x["accountId"]})',
            key="ib_account_overview",
        )
        if "ledger" in account:
            ledger = account["ledger"]["BASE"]
            c1, c2, c3, c4, c5, c6 = st.columns(6)
            c1.metric("Net Liquidation", f'{ledger.get("netliquidationvalue"):,.0f}')
            c2.metric("Cash Balance", f'{ledger.get("cashbalance"):,.0f}')
            c3.metric("Interest", f'{ledger.get("interest"):,.0f}')
            c4.metric("Dividends", f'{ledger.get("dividends"):,.0f}')
            c5.metric("Unrealized PnL", f'{ledger.get("unrealizedpnl"):,.0f}')
            c6.metric("Realized PnL", f'{ledger.get("realizedpnl"):,.0f}')
            c1.metric("Stock Value", f'{ledger.get("stockmarketvalue"):,.0f}')
            c2.metric("Option Value", f'{ledger.get("stockoptionmarketvalue"):,.0f}')
            c3.metric("Future Value", f'{ledger.get("futuremarketvalue"):,.0f}')
            c4.metric(
                "Future Option Value",
                f'{ledger.get("futureoptionmarketvalue"):,.0f}',
            )
            c5.metric("Warrant Value", f'{ledger.get("warrantsmarketvalue"):,.0f}')
            c6.metric(
                "Commodity Value", f'{ledger.get("commoditymarketvalue"):,.0f}'
            )
            c1.metric("Bond Value", f'{ledger.get("tbondsmarketvalue"):,.0f}')
            c2.metric("Bill Value", f'{ledger.get("tbillsmarketvalue"):,.0f}')
            c3.metric(
                "Corporate Bond Value",
                f'{ledger.get("corporatebondsmarketvalue"):,.0f}',
            )
            c4.metric("Fund Value", f'{ledger.get("funds"):,.0f}')
            c5.metric("Money Fund Value", f'{ledger.get("moneyfunds"):,.0f}')
            c6.metric("Crypto Value", f'{ledger.get("cryptocurrencyvalue"):,.0f}')
        if "performance" in account:
            ...
            nav_s = pd.Series(
                nav["data"][0]["navs"],
                index=pd.to_datetime(nav["dates"], format="%Y%m%d"),
            )
            ...
            cps_s = pd.Series(
                cps["data"][0]["returns"],
                index=pd.to_datetime(cps["dates"], format="%Y%m%d"),
            )
...
def show_ib_tearsheet(tab, info):
    with tab:
        ...
        if "performance" in account:
            ...
            qs.reports.html(
                (cps_s + 1).pct_change().dropna(),
                benchmark="SPY",
                rf=0.0,
                display=False,
                output=temp,
                title=f'{account["displayName"]} ({account["accountId"]})',
            )
            with open(temp) as f:
...
def show_broker_account_portfolio_and_trades(account: BrokerAccount):
    tab1, tab2, tab3, tab4, tab5 = st.tabs(
        ["Overview", "Recent trades", "Orders", "Account info", "Performance"]
    )
    try:
